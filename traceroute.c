...
struct proto proto_v4 = {
    icmpcode_v4, recv_v4, NULL, NULL, NULL, NULL, 0,
    IPPROTO_ICMP, IPPROTO_IP, IP_TTL
};

struct proto proto_v6 = {
    icmpcode_v6, recv_v6, NULL, NULL, NULL, NULL, 0,
...
void traceloop(void)
{
    ...
    for (ttl = 1; ttl <= max_ttl && done == 0; ttl++) {
        rc = setsockopt(sendfd, pr->ttllevel, pr->ttloptname, &ttl, sizeof(int));
        if (rc < 0) {
        ...
        for (probe = 0; probe < nprobes; probe++) {
            ...
            rc = (int)sendto(sendfd, sendbuf, datalen, 0, pr->sasend, pr->salen);
            if (rc != (int)datalen) {
                perror("sendto error");
                exit(1);
            }
